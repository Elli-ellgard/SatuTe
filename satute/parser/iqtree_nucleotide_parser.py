...
class NucleotideParser(BaseParser):
    ...
    def construct_rate_matrix(self, state_frequencies: List[float])->np.array:
        ...
        n = self.find_dimension_by_rate_matrix_parsing(start_index=start_index, file_content=self.file_content)
    ...
    def normalize_rate_matrix(self ,rate_matrix: np.ndarray, state_frequencies: List[float], n: int) -> np.ndarray:
        average_rate = 0
        for i in range(n):
            rate_matrix[i, i] = -np.sum(rate_matrix[i, :]) + rate_matrix[i, i]
            average_rate += rate_matrix[i, i] * state_frequencies[i]
        if average_rate == 0:
            raise ValueError("Division by zero in normalization due to zero average rate.")
        return -rate_matrix / average_rate

    def assemble_rate_matrix_from_rates_and_frequencies(self,n: int, rates: List[float], state_frequencies: List[float]) -> np.ndarray:
        expected_number_of_rates = n * (n - 1) // 2
        if len(rates) != expected_number_of_rates:
            raise ValueError(f"Expected {expected_number_of_rates} rates, got {len(rates)}.")
