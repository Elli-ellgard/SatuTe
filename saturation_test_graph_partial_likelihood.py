...
if __name__ == "__main__":
    ...
    alignment_file = "./test/octo-kraken-msa-test/example.phy"
    alignment = read_alignment_file(alignment_file)
    t = parse_newick_file("./test/octo-kraken-msa-without-gamma/example.phy.treefile")
    t = name_nodes_by_level_order(t)
    number_rate = 1

    print("Reading Rate Matrix")
    RATE_MATRIX, STATIONARY_DISTRIBUTION = parse_rate_matrices(
        4, "./test/octo-kraken-msa-without-gamma/example.phy"
    )
    print(RATE_MATRIX)
    rate_matrix = RateMatrix(RATE_MATRIX)

    if number_rate == 1:
        # Case: Single Rate Analysis
        print("Number Rate: ", number_rate)
        # Calculate partial likelihoods for sites using the overall rate matrix
        ...
        # Case: Multiple Rate Analysis

        # Read site probabilities from file
        ...
        # Split the multiple sequence alignment (MSA) into rate categories
        valid_category_rates = split_msa_into_rate_categories_in_place(
            site_probability, "./Clemens/example_3", alignment
        )

        # Parse category rates from an IQ-TREE output file
        ...
        # Iterate through valid category rates for calculations
        for valid_category_rate in valid_category_rates:
            # Print the category rate being calculated
            print("Calculating for rate ", valid_category_rate)

            # Retrieve relative rate factor for the current category
            rate = category_rates_factors[valid_category_rate - 1]["Relative_rate"]

            # Rescale the branch lengths of the tree based on the relative rate
            rescaled_tree = rescale_branch_lengths(t, rate)

            # Read the sub-alignment corresponding to the current rate category
            ...
            # Calculate partial likelihoods for sites using the rescaled tree and rate matrix
            ...
            # Print results for each edge and its associated partial likelihoods
