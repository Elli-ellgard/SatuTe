...
def parse_state_frequencies(file_content):
    ...
    """
    index = next(
        (idx for idx, line in enumerate(file_content) if "State frequencies:" in line),
        None,
    )
    ...
    start_idx = next(
        (idx for idx, line in enumerate(file_content) if "Rate matrix Q:" in line),
        None,
    )
...
def file_path():
    ...
    return os.path.join(data_dir_path, "example1.iqtree")
