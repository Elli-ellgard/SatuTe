...
class TestIqTreeHandler(unittest.TestCase):
    def setUp(self):
        self.iq_tree_path = "iqtree2"
        self.logger = Mock()
        self.iq_tree_handler = IqTreeHandler(self.iq_tree_path, self.logger)
    ...
    def test_run_iqtree_with_arguments_logs_output_and_error(self, mock_run):
        ...
        expected_command = f"{self.iq_tree_path} {' '.join(arguments)} {' '.join(extra_arguments)}"

        mock_run.return_value.stdout = "IQ-TREE output"
        mock_run.return_value.stderr = "IQ-TREE error"

        self.iq_tree_handler.run_iqtree_with_arguments(arguments, extra_arguments)
