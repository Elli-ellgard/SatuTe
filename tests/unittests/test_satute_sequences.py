...
from satute.sequences import check_if_tree_has_same_taxa_as_msa, TaxaMismatchError
...
class TestCheckIfAlignmentHasSameTaxaAsMSA(unittest.TestCase):
    ...
    def test_matching_taxa(self):
        try:
            check_if_tree_has_same_taxa_as_msa(self.alignment, self.tree_matching)
    ...
    def test_non_matching_taxa(self):
        with self.assertRaises(TaxaMismatchError) as cm:
            check_if_tree_has_same_taxa_as_msa(self.alignment, self.tree_non_matching)
    ...
    def test_partial_matching_taxa(self):
        with self.assertRaises(TaxaMismatchError) as cm:
            check_if_tree_has_same_taxa_as_msa(
                self.alignment, self.tree_partial_matching
            )
